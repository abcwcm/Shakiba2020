...
Principle component analysis (PCA) of RNA-seq data. Each point represents a biological replicate.  Top 500 most variable genes based on VS-transformed data.
...
#genes_to_highlight <- c("Il7r","Runx1","Fas","Tnfrsf22","Tcf7","Klf2","Mapk11","Tnf","Etv1","Id3","Rora","Tet1","Tox","Mafb","Entpd1","Cd244","Cd300a","Hdac9","Il4ra","Tcf7l2","Tcf4","Irf5","Itgae")
...
```{r ext_fig4a, message=FALSE, eval=TRUE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, results='hide', fig.height=6, fig.width=5, fig.align="center"}
counts <- read.table(file = "gene.counts.txt", header = TRUE, check.names=FALSE, row.names=1)
decoderFile <- "decoder.txt"
decoder.data <- read.table(decoderFile,header=T,stringsAsFactors=F,sep="\t")
decoder.data$condition <- factor(decoder.data$group.ID)
...
#pheatmap(cpms[row.names( subset(deseq2.res.sig.TN_F6_vs_T.D4_sig, abs(log2FoldChange)> 1) ),grep("T-|N[123]", colnames(cpms))], scale="row", fontsize=8, show_rownames=F, color = my_colors, main = "T.N_T.F6 vs. T.D4, padj < 0.10, abs(logFC) > 1", cutree_row=4, annotation_row = annotation_row, cluster_cols = F)
...
pheatmap(cpms[ordered_row, column_order], scale="row", fontsize=8, show_rownames=F, color = my_colors, main = "T.N_T.F6 vs. T.D4, padj < 0.10, abs(logFC) > 1", cutree_row=4, cluster_cols = F, cluster_rows = F, gaps_row = c(226,317,518)) 
...
```{r ext_fig4b, message=FALSE, eval=T, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, results='hide', fig.height=8, fig.width=8, fig.align="center"}
counts <- read.table(file = "gene.counts.txt", header = TRUE, check.names=FALSE, row.names=1)
decoderFile <- "decoder.txt"
decoder.data <- read.table(decoderFile,header=T,stringsAsFactors=F,sep="\t")
decoder.data$condition <- factor(decoder.data$group.ID)
...
```{r extfig4d,  message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, context="data"}
...
```{r extfig4f_up, message=FALSE, eval=TRUE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, results='hide',  fig.align="center"}
...
## Extended Fig5a (atac-seq)


Fragment length distribution plots (bp on x axis and read count on y axis). 

```{r ext_fig5a, message=FALSE, eval=TRUE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, results='hide',fig.width=10, fig.height=5.5, fig.align="center"}
frags.raw <- fread("fragLengthsStats_finalBams.txt")
...
```{r ext_fig5b, message=FALSE, eval=TRUE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, results='hide', fig.width=10, fig.height=5.5, fig.align="center"}
bs.plot <- fread("atac_map_stats.txt")
...
## Extended Fig6a (atac-seq)
Number of chromatin accessibility changes in TIL encountering high- vs. low-affinity tumor antigen.

```{r ext_fig6a,  message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, context="data"}
...
## Extended Fig6b (atac-seq)
Pie chart showing the proportion of reproducible ATAC-Seq peaks in exonic, intronic, intergenic and promoter regions.


```{r ext_fig6b,  eval=T,message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, context="data"}
DB <- readRDS("DB.noXY.RDS")
...
```{r ext_fig6c_left, message=FALSE, eval=TRUE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, results='hide', fig.height=8, fig.align="center"}
DB <- readRDS("DB.noXY.RDS")
...
```{r ext_fig6d, message=FALSE, eval=TRUE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, results='hide', fig.height=8, fig.align="center"}
DB <- readRDS("DB.noXY.RDS")
...
```{r ext_fig6e,message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE, comment=FALSE, context="data"}
library(rGREAT)
DB <- readRDS("DB.noXY.RDS")
