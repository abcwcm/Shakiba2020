...
BAM_COVERAGE_NRM_TO_1X = 2652783500  # mouse https://deeptools.readthedocs.io/en/develop/content/feature/effectiveGenomeSize.html
EFFECTIVE_GENOME_SIZE = mm # mouse
...
final_bam_qc : $(addsuffix _PF.final.bam.idxstat.log, $(SAMPLES))
...
$1_PF.maxL.bam: $(call find-fastq-files,$1)
	$(BWA) aln -t 8 $(REFERENCE_DIR)/$(REFERENCE_PREFIX) $$(notdir $$(wildcard $1*_R1.fastq.gz)) > $$(basename $$(basename $$(wildcard $1*_R1.fastq.gz))).sai ; \
	$(BWA) aln -t 8 $(REFERENCE_DIR)/$(REFERENCE_PREFIX) $$(notdir $$(wildcard $1*_R2.fastq.gz )) > $$(basename $$(basename $$(wildcard $1*_R2.fastq.gz))).sai ; \
...
%_PF.final.bam: %_PF.rmdup.bam
	$(SAMTOOLS) view -h $< | egrep 'chr[0-9]+|^@HD|^@PG|XT:A:U'  | $(SAMTOOLS) view -bS - | $(SAMTOOLS) sort -n -O bam - > $@.tmp ; $(SAMTOOLS) fixmate -r $@.tmp - | $(SAMTOOLS) view -F 1804 -f 2 -u - | $(SAMTOOLS) sort - > $@ && rm $@.tmp
